...
class PlaybookExecutor:
        ...
        try:
            ...
            for playbook_path in self._playbooks:
                ...
                for play in plays:
                    ...
                    setattr(
                        play, 'vars_prompt', templar.template(
                            play.vars_prompt))

                    # FIXME: this should be a play 'sub object' like
                    # loop_control
                    if play.vars_prompt:
                            ...
                            if vname not in self._variable_manager.extra_vars:
                                if self._tqm:
                                    ...
                                    play.vars[vname] = display.do_var_prompt(
                                        vname, private, prompt, encrypt, confirm, salt_size, salt, default, unsafe)
                    ...
                    if self._tqm is None:
                        ...
                        for batch in batches:
                            # restrict the inventory to the hosts in the
                            # serialized batch
                            self._inventory.restrict_to_hosts(batch)
                            # and run it...
                            result = self._tqm.run(play=play)

                            # break the play if the result equals the special
                            # return code
                            ...
                            # batch failed
                            failed_hosts_count = len(self._tqm._failed_hosts) + len(
                                self._tqm._unreachable_hosts) - (previously_failed + previously_unreachable)
                            ...
                            previously_unreachable += len(
                                self._tqm._unreachable_hosts) - previously_unreachable
    ...
    @staticmethod
    def exec_timeout_handler(sig, frame):
        if sig == 14:
            display.debug('Task: execution timed out!')
            raise AnsibleTaskTimeoutError("Execution time out")
